...
#todo, add NUPACK controllability!
...
'''
def tb_assign_nicks(face_data):
    
    #What the shit does this function do?

    breaks = tb_get_break_index(face_data)
    V_vals = [i.e for i in face_data]
    edge_dict = {}
    for face1,edges_1 in enumerate(E_vals):
        for edge in edges_1:
            for face2,edges_2 in enumerate(E_vals):
...
def get_segments(FNAME, LENGTH_OF_SMALLEST, SPACERS,nicks=1,overhangs = None,turberfieldnicks = 0,ldmoverhangs = None):
    ...
    ##NICK ASSIGNMENT WITH SEEMAN NICKS
    ...
    ##NICKS ASSIGNMENT WITH TURBERFIELD NICKS

    if tn:
        #this function is completely wrong.
        breaks = tn_get_break_index(face_data)
        breakpoint()
    ...
    for f in face_data:
        ...
        for con in f.connections:
            ### See if we have an overhang 365G
            overhang_here = False
            ...
            if type(ldmoverhangs) != type(None):
                #see if we're at an LMD breakpoint here...
            ...
            if SPACERS != 0:
                ...
                if overhang_here:
                    ...
                    if ss_here == 0: 
                        ...
                        # TODO KILL CONNECTIVITY here IF WE'RE IN THE OVERHANG FILE!
