...
def __main__():
    ...
    TURBERFIELD_NICKS = int(args.turberfieldnicks)
    ...
    ldmoverhangs = None
    ldmoverhangfile = args.ldmoverhangfile
    
    if type(ldmoverhangfile) != type(None):
        ldmoverhangs= read_overhang_file(ldmoverhangfile)

    #breakpoint()

    segs_list = segment_maker_oo.get_segments(FNAME, LENGTH_OF_SMALLEST, SPACERS,nicks = args.nicks,overhangs = overhangs,turberfieldnicks=TURBERFIELD_NICKS,ldmoverhangs = ldmoverhangs)
